...
// produce separate Resource classes. This stub restores the old API surface for domains.
...
namespace Azure.ResourceManager.EventGrid
{
        ...
        IPersistableModel<EventGridPrivateEndpointConnectionData>
    {
        ...
        internal EventGridDomainPrivateEndpointConnectionResource(ArmClient client, EventGridPrivateEndpointConnectionData data)
            : base(client, data.Id)
        {
            _data = data;
            HasData = true;
        ...
        internal EventGridDomainPrivateEndpointConnectionResource(ArmClient client, ResourceIdentifier id)
            : base(client, id)
        {
        }
        ...
        {
            return new ResourceIdentifier($"/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.EventGrid/domains/{parentName}/privateEndpointConnections/{privateEndpointConnectionName}");
        }
        ...
        {
            using DiagnosticScope scope = ClientDiags.CreateScope("EventGridDomainPrivateEndpointConnectionResource.Get");
            scope.Start();
            try
            {
                RequestContext context = new RequestContext { CancellationToken = cancellationToken };
                HttpMessage message = RestClient.CreateGetRequest(Guid.Parse(Id.SubscriptionId), Id.ResourceGroupName, "domains", Id.Parent.Name, Id.Name, context);
        ...
        public virtual async Task<ArmOperation<EventGridDomainPrivateEndpointConnectionResource>> UpdateAsync(WaitUntil waitUntil, EventGridPrivateEndpointConnectionData data, CancellationToken cancellationToken = default)
        {
            ...
            using DiagnosticScope scope = ClientDiags.CreateScope("EventGridDomainPrivateEndpointConnectionResource.Update");
            scope.Start();
            try
            {
                RequestContext context = new RequestContext { CancellationToken = cancellationToken };
                HttpMessage message = RestClient.CreateUpdateRequest(Guid.Parse(Id.SubscriptionId), Id.ResourceGroupName, "domains", Id.Parent.Name, Id.Name, EventGridPrivateEndpointConnectionData.ToRequestContent(data), context);
        ...
        {
            using DiagnosticScope scope = ClientDiags.CreateScope("EventGridDomainPrivateEndpointConnectionResource.Delete");
            scope.Start();
            try
            {
                RequestContext context = new RequestContext { CancellationToken = cancellationToken };
                HttpMessage message = RestClient.CreateDeleteRequest(Guid.Parse(Id.SubscriptionId), Id.ResourceGroupName, "domains", Id.Parent.Name, Id.Name, context);
                Response response = await Pipeline.ProcessMessageAsync(message, context).ConfigureAwait(false);
