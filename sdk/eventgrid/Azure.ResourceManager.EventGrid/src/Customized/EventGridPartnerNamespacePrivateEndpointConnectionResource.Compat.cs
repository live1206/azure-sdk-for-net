...
namespace Azure.ResourceManager.EventGrid
{
        ...
        IPersistableModel<EventGridPrivateEndpointConnectionData>
    {
        ...
        internal EventGridPartnerNamespacePrivateEndpointConnectionResource(ArmClient client, EventGridPrivateEndpointConnectionData data)
            : base(client, data.Id)
        {
            _data = data;
            HasData = true;
        ...
        internal EventGridPartnerNamespacePrivateEndpointConnectionResource(ArmClient client, ResourceIdentifier id)
            : base(client, id)
        {
        }
        ...
        {
            return new ResourceIdentifier($"/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.EventGrid/partnerNamespaces/{parentName}/privateEndpointConnections/{privateEndpointConnectionName}");
        }
        ...
        {
            using DiagnosticScope scope = ClientDiags.CreateScope("EventGridPartnerNamespacePrivateEndpointConnectionResource.Get");
            scope.Start();
            try
            {
        ...
        public virtual async Task<ArmOperation<EventGridPartnerNamespacePrivateEndpointConnectionResource>> UpdateAsync(WaitUntil waitUntil, EventGridPrivateEndpointConnectionData data, CancellationToken cancellationToken = default)
        {
            ...
            using DiagnosticScope scope = ClientDiags.CreateScope("EventGridPartnerNamespacePrivateEndpointConnectionResource.Update");
            scope.Start();
            try
            {
                RequestContext context = new RequestContext { CancellationToken = cancellationToken };
                HttpMessage message = RestClient.CreateUpdateRequest(Guid.Parse(Id.SubscriptionId), Id.ResourceGroupName, "partnerNamespaces", Id.Parent.Name, Id.Name, EventGridPrivateEndpointConnectionData.ToRequestContent(data), context);
        ...
        {
            using DiagnosticScope scope = ClientDiags.CreateScope("EventGridPartnerNamespacePrivateEndpointConnectionResource.Delete");
            scope.Start();
            try
            {
                RequestContext context = new RequestContext { CancellationToken = cancellationToken };
                HttpMessage message = RestClient.CreateDeleteRequest(Guid.Parse(Id.SubscriptionId), Id.ResourceGroupName, "partnerNamespaces", Id.Parent.Name, Id.Name, context);
                Response response = await Pipeline.ProcessMessageAsync(message, context).ConfigureAwait(false);
